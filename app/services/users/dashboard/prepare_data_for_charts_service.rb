  ...
  module Dashboard
    class PrepareDataForChartsService < ApplicationService
      ...
      def expenditure_for_month
        group_calculate_service_price(:price)
      end
      ...
      def group_calculate_service_price(field)
        @services.group_by { |service| service.created_at.month }.map do |month, services|
          [services.sum(&field), month]
        end
      end
